        ...
        >
            <div>
                {yearDiagnostics.map(({ year, diagnostics }) => {
                    return (
                        <div className="yearDiagnostics" key={`${year}`}>
                            ...
                            {diagnostics.map(({ date, projectionType, euTirads }) => {
                                return (
                                    <Flex
                                        ...
                                        vertical
                                        gap={10}
                                        key={`${date}-${projectionType}-${euTirads}`}
                                    >
                                        <div className="date">
                                            Дата:{" "}
                                        ...
                                        <div className="projectionType">
                                            Тип проекции:{" "}
                                            <span className="border">{projectionType}</span>
                                        </div>
                                        <div className="euTirads">
                                            EU-TIRADS: <span className="border">{euTirads}</span>
                                        </div>
                                    </Flex>
                                );
                            })}
