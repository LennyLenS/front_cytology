...
export default function Chat() {
    ...
    const sendMessage = useCallback(() => {
        setMessages([
            ...messages,
            { id: messages.length, own: true, datetime: dayjs(), textContent: textarea_message },
        ]);
    ...
    return (
        <Page className="page chat_page">
            <DiagnosticDrawer
                ...
                handleOk={handleOk}
            />
            <Button className="viewing_diagnostics_button" type="primary" onClick={showDrawer}>
                <LeftOutlined />
            ...
            <div className="message_list_container">
                <Flex gap={20} className="message_list" vertical>
                    {messages.map(({ own, datetime, textContent, id }) => {
                        return (
                            <div className={clsx("message", { own_message: own })} key={id}>
                                <Flex gap={10} vertical>
                                    <div className="datetime">
                                        {datetime.format("DD/MM/YYYY")} - Цыгулева Ксения
                                        Владимировна
                                    </div>
